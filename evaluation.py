...
def metric_calculation(y_pred, y_true, gamma=0.4,mu = 0.8, zeta = 0.4, norm=False,dataset_name="", model_name=""):#alpha=0.4, beta = 0.8,gamma=0.4,rho = 11.4   
    ...
    if norm:
        scaler = load(open(f'{config.DATA_FOLDER}/{dataset_name}/stanard_scaler.pkl', 'rb'))
    ...
    return [rmse,range_gamma,avg_rse], [norm_error_mean, range_zeta, avg_ne],[smape,range_mu,avg_sape], y_true,y_pred#mae, lyapunov_error_mean,  njpe,

def evaluation(y_pred, y_true,performance=None,gamma=0.4,mu = 0.8, zeta = 0.4, norm=False,dataset_name="", model_name=""): 
    rmse, ne,smape, y_true,y_pred = metric_calculation(y_pred, y_true, gamma,mu, zeta,norm,dataset_name,model_name)
    
    if performance is None:
        performance_idx = ["$\mathbb{E}_\gamma$","$\mathbb{P}_\gamma$","$\mathbb{E}_\zeta$","$\mathbb{P}_\zeta$","$\mathbb{E}_\mu$","$\mathbb{P}_\mu$"]
...
def get_pred_bls_elm(saved_folder,train_samples,test_inputs, pred_steps, x_y_lag):
    if os.path.exists(f"{config.RESULT_FOLDER}/{saved_folder}/bls.npy") and os.path.exists(f"{config.RESULT_FOLDER}/{saved_folder}/elm.npy"): 
        pred_bls = np.load(f"{config.RESULT_FOLDER}/{saved_folder}/bls.npy")
        pred_elm = np.load(f"{config.RESULT_FOLDER}/{saved_folder}/elm.npy")
    else:
        assert len(train_samples.shape) == 3, "train samples should be 3 dimensions: (samples, time steps, feature dimensions)"
        assert len(test_inputs.shape) == 3, "test input sequence should be 3 dimensions: (samples, time steps, feature dimensions)"
        ...
        pred_elm =  elm.inference(test_x, pred_steps*nv_te,x_y_lag*nv_te).reshape(pred_shape)
...
def get_pred_mt(saved_folder, test_inputs, pred_steps, x_y_lag): 
    if os.path.exists(f"{config.RESULT_FOLDER}/{saved_folder}/mt.npy"): 
        ...
        mtran = MusicTransformer(test_inputs.shape[-1], f"{config.MODEL_FOLDER}/{saved_folder}",x_y_lag=x_y_lag)
...
def get_pred_tf(saved_folder, test_inputs, pred_steps, x_y_lag): 
    if os.path.exists(f"{config.RESULT_FOLDER}/{saved_folder}/tf.npy"): 
        ...
        tf = TeacherForcing(test_inputs.shape[-1], f"{config.MODEL_FOLDER}/{saved_folder}",x_y_lag=x_y_lag)    
...
def get_pred_naive(saved_folder, test_inputs, pred_steps, x_y_lag,k=20): 
    if os.path.exists(f"{config.RESULT_FOLDER}/{saved_folder}/hf{k}naive.npy"): 
        ...
        hf = HorizonForcing(test_inputs.shape[-1], f"{config.MODEL_FOLDER}/{saved_folder}",x_y_lag=x_y_lag, k=k,suffix = "naive")  
        print(hf) 
...
def get_pred_dss(saved_folder, test_inputs, pred_steps, x_y_lag,decay = "is"): 
    if os.path.exists(f"{config.RESULT_FOLDER}/{saved_folder}/dss_{decay}.npy"): 
        ...
        ss = DiscontinuousScheduledSampling(test_inputs.shape[-1], f"{config.MODEL_FOLDER}/{saved_folder}",x_y_lag=x_y_lag,ts = test_inputs.shape[1],decay = decay)    
        pred_ss = ss.inference(test_inputs, pred_steps=pred_steps)
        if pred_ss is not None:
...
def get_pred_hf(saved_folder, test_inputs, pred_steps, x_y_lag, k=5): 
    if os.path.exists(f"{config.RESULT_FOLDER}/{saved_folder}/hf{k}.npy"): 
        ...
        hf = HorizonForcing(test_inputs.shape[-1], f"{config.MODEL_FOLDER}/{saved_folder}",x_y_lag=x_y_lag, k=k)  
        print(hf) 
        pred_hf = hf.inference(test_inputs, pred_steps=pred_steps) 
        if pred_hf is not None:
...
BENCHMARKING   = {"BLS":{"fun":get_pred_bls_elm},
          "ELM":{"fun":get_pred_bls_elm},
        #   "SSES":{"fun":get_pred_ss},
          "SSFS":{"fun":get_pred_dss,"factor": "is"},
          "MTF":{"fun":get_pred_mt},
...
MODE = {"ett":ETT_ABLATION,"hf":HF_STUDY,"benchmark":BENCHMARKING, "all":ALL, "recurrent": RECURRENT, "transformers": TRANSFORMERS}

def _plot_effective_range(dataframe,ax,palette,y_lim,x_lim,y_label= "Error $\gamma$", delta=None, anchor=None,fontsize=24):
    g = sns.lineplot(x='index', y='value', hue='variable', palette=palette, ci="sd",linewidth = 5,
            data=pd.melt(dataframe,"index"), ax =ax)
    g.set(ylim=y_lim)
    g.set(xlim=(0, x_lim))
    if delta:
...
def plot_comparison(study_mode, figsize, fontsize = 24, linewidth=3, anchor = (0,-0.05,1,1), legsize=2, fold= 1, x_y_lag=1 ):
    normalization= False
    result = {}  
    ...
    def get_results(pred, y_true, perform, rmse_out, ne_out, smape_out,idx, dataset_name, model_name, norm):
        perform,rmse, ne, smape,y_true,pred_bls =  evaluation(pred,y_true,perform,gamma,mu, zeta,norm,dataset_name,model_name)
    ...
    for idx_data, (sys, params) in enumerate(config.EXP_SETTING.items()):
        key, x_steps, inference_steps= params["title"], params["input_steps"], params["inference_steps"]
        gamma, zeta, mu, y_lim = params["gamma"], params["zeta"], params["mu"], params["y_lim"][study_mode]
        
        dataset_name = config.get_dataset_name(sys)
        train_samples,test_inputs, y_true = load_model_config(dataset_name,fold,normalization, x_steps)
        ...
        for idx_pred, (name, vals) in enumerate(pred_funs.items()):  
            if rmse_out is None:    
                rmse_out , ne_out , smape_out = np.empty((inference_steps,num_models)), np.empty((inference_steps,num_models)), np.empty((inference_steps,num_models))
                rmse_out[:], ne_out[:], smape_out[:] = np.nan, np.nan, np.nan   
            if name == "BLS": 
                if pred_bls is None:
                    pred_bls, pred_elm = vals["fun"](saved_folder, train_samples,test_inputs, inference_steps, x_y_lag)
                perform, rmse_out, ne_out, smape_out =  get_results(pred_bls[:,x_steps:x_steps+inference_steps],
                        y_true,perform,rmse_out, ne_out, smape_out,idx_pred, dataset_name, "BLS", normalization)
                colors.append(config.COLOR_BANK["BLS"]) 
            elif name == "ELM":
                if pred_elm is None:
                    pred_bls, pred_elm = vals["fun"](saved_folder, train_samples,test_inputs, inference_steps, x_y_lag)
                perform, rmse_out, ne_out, smape_out =  get_results(pred_elm[:,x_steps:x_steps+inference_steps],
                        y_true,perform,rmse_out, ne_out, smape_out,idx_pred, dataset_name, "ELM", normalization)
                colors.append(config.COLOR_BANK["ELM"]) 
            elif name == "IF":
                pred, true = vals["fun"](sys,inference_steps)
                perform, rmse_out, ne_out, smape_out =  get_results(pred,
                        true,perform,rmse_out, ne_out, smape_out,idx_pred, dataset_name, "Informer", normalization)
                colors.append(config.COLOR_BANK["IF"]) 
            elif name == "AF":
                pred, true = vals["fun"](sys,inference_steps)
                perform, rmse_out, ne_out, smape_out =  get_results(pred,
                        true,perform,rmse_out, ne_out, smape_out,idx_pred, dataset_name, "Autoformer", normalization)
                colors.append(config.COLOR_BANK["AF"]) 
                
            else:
                if vals.get("factor") is not None:
                    pred = vals["fun"](saved_folder, test_inputs, inference_steps, x_y_lag, vals["factor"])
                else:
                    pred = vals["fun"](saved_folder, test_inputs, inference_steps, x_y_lag)
                if pred is not None:
                    perform, rmse_out, ne_out, smape_out =  get_results(pred[:,x_steps:x_steps+inference_steps],
                            y_true,perform,rmse_out, ne_out, smape_out,idx_pred, dataset_name, name, normalization) 
        ...
        _plot_effective_range(rmse_out_df,axs[0,idx_data],palette,y_lim[0],inference_steps,y_label= "$Error$ $\gamma$", fontsize= fontsize)
        _plot_effective_range(ne_out_df,axs[1,idx_data],palette,y_lim[1],inference_steps,y_label= "$Error$ $\zeta$", fontsize= fontsize)
        _plot_effective_range(smape_out_df,axs[2,idx_data],palette,y_lim[2],inference_steps,y_label= "$Error$ $\mu$", fontsize= fontsize) 
        # if idx_data!=0:
    ...
    leg = fig.legend(handles, labels, loc=('lower center'),bbox_to_anchor = anchor,bbox_transform = plt.gcf().transFigure, ncol=num_models//2+1,labelspacing=0.1, fontsize=fontsize+5,frameon=False)
...
def load_model_config(dataset_name,fold,norm=False, x_steps=100):
    ...
    path = os.path.join(f"{config.DATA_FOLDER}/{dataset_name}/indices",f'{config.FOLDS}fold.{fold}_train_index.npy')  
