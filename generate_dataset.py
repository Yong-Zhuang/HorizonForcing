...
def build_data_series(series_index):
    global _DERIVATIVE
...
parser.add_argument(
    "-n", type=int, default=15000, help="Number of training examples in dataset."
)
parser.add_argument(
    "-n_test", type=int, default=500, help="Number of testing examples in dataset."
)
parser.add_argument("-ns", type=int, default=1000, help="Number of steps per example.")
parser.add_argument(
    "-dt", type=float, default=0.05, help="Time step magnitude, delta-t."
)
parser.add_argument(
    "-o", "--output_dir", default="./data/", help="Path where data should be saved."
)
parser.add_argument(
    "-atol", type=float, default=1e-8, help="Absolute tolerance for IVP solver."
)
parser.add_argument(
    "-rtol", type=float, default=1e-8, help="Relative Tolerance for IVP solver."
)
if __name__ == "__main__":
    ...
    if dataset_name == "lorenz":
        meta = _LORENZ_META
        _DERIVATIVE = get_lorenz_derivative(*meta["params"])
    else:
        meta = _ROSSLER_META
        _DERIVATIVE = get_rossler_derivative(*meta["params"])

    dimension = meta["dimension"]

    # Allocate memory for datasets.
    _FINAL_ARRAY = np.ctypeslib.as_ctypes(np.zeros((total_n, n_steps, dimension)))

    _SHARED_ARRAY = sharedctypes.RawArray(_FINAL_ARRAY._type_, _FINAL_ARRAY)
    ...
    # Names of data files.
    train_output_file = Path(output_dir) / (f"X_train_{dataset_name}.npy")

    test_output_file = Path(output_dir) / (f"X_test_{dataset_name}.npy")
