...
def generate_train_test_indices(training_samples, x_steps, dataset_name):
    print(f"dataset shape: {training_samples.shape}")
    ...
    for train_index, val_index in kfold.split(
        train_data[:, :x_steps, 0], train_data[:, x_steps, 0]
    ):
        path = os.path.join(
            indices_folder, f"{config.FOLDS}fold.{fold_idx}_train_index"
        )
...
def training(training_samples, x_steps, dataset_name, fold_idx, x_y_lag, norm):
    ...
    for estimator_name, setting in config.ESTIMATORS.items():
        ...
        if estimator_name == f"GRU_baseline":
            ...
            estimator = HorizonForcing(
                pred_dim, saved_folder, x_y_lag=x_y_lag, k=setting["k"], lr=lr
            )
            ...
            estimator = DiscontinuousScheduledSampling(
                pred_dim,
                saved_folder,
                x_y_lag=x_y_lag,
                ts=x_steps,
            ...
            if estimator_name == "GRU_hf_5":
                ...
                base_model = HorizonForcing(
                    pred_dim, saved_folder, x_y_lag=x_y_lag, k=5, lr=lr
                )
                base_model.load_weights()
                ...
                base_model = HorizonForcing(
                    pred_dim, saved_folder, x_y_lag=x_y_lag, k=10, lr=lr
                )
                base_model.load_weights()
                gru = base_model.time_gru.gru
                output_dense = base_model.time_gru.output_dense
                ...
                base_model = HorizonForcing(
                    pred_dim, saved_folder, x_y_lag=x_y_lag, k=15, lr=lr
                )
                base_model.load_weights()
                gru = base_model.time_gru.gru
                output_dense = base_model.time_gru.output_dense
            ...
            estimator = MusicTransformer(
                ...
                saved_folder,
                x_y_lag=x_y_lag,
                latent_dim=setting["num_head"] * 64,
...
parser.add_argument(
    ...
    default="0",
    required=True,
    help="gpu index, using which GPU",
)
parser.add_argument(
    "-s",
    "--system",
...
parser.add_argument(
    "-fold", metavar="int", type=int, default=1, help="fold idx: 0,1,2,3,or 4"
)
parser.add_argument(
    "-lag",
    metavar="int",
    type=int,
    default=1,
    help="a lag time steps between input and output, value should be less or equal than the input steps xs",
)
