...
class RelativeGlobalAttention(keras.layers.Layer):
    ...
    def _qe_masking(qe):
        ...
        mask = tf.sequence_mask(
            tf.range(qe_shape[-1] -1, qe_shape[-1] - qe_shape[-2] -1, -1), qe_shape[-1])
    ...
    def _skewing(self, tensor: tf.Tensor):
        ...
        reshaped = tf.reshape(padded, shape=[-1, padded_shape[1], padded_shape[-1], padded_shape[-2]])
        Srel = reshaped[:, :, 1:, :]
        return Srel
...
class Encoder(keras.layers.Layer):
    def __init__(self, num_layers, d_model, rate=0.1, max_len=None,PositionEmbedding=True):
...
class HorizonForcingGRULayer(keras.layers.Layer):
    ...
    def call(self, inputs, verbose=False):
        ...
        def gru_step(inputs, states):
            ...
            gru_outs, state = self.gru(pred,initial_state=inputs) #=> (batch_size, hidden)
        ...
        def create_inital_state(inputs, hidden_size):
            ...
            fake_state = K.zeros_like(inputs)  # <= (batch_size, enc_seq_len, hidden_size)
            fake_state = K.sum(fake_state, axis=[1, 2])  # <= (batch_size)
            fake_state = K.expand_dims(fake_state)  # <= (batch_size, 1)
            fake_state = K.tile(fake_state, [1, hidden_size])  # <= (batch_size, hidden_size)
            return fake_state

        fake_state = create_inital_state(gru_output_seq, self.out_dim + gru_output_seq_shape[-1])
...
class ScheduledSamplingGRULayer(keras.layers.Layer):
    ...
    def call(self, inputs, verbose=False):
        ...
        """ Computing 1 step inference outputs """
        _, outputs, _ = K.rnn(gru_step, gru_output_seq, [fake_state])
        preds1, gru_states = outputs[:,:,:self.out_dim], outputs[:,:,self.out_dim:]
        ...
        print(f"preds1 shape {preds1.shape}, preds2 shape {preds2.shape}, sampling_size is {sampling_size}")
        preds = tf.concat([preds1[:,:sampling_size+1],preds2[:,sampling_size:]],axis = 1)
...
class ScheduledSamplingLayer(keras.layers.Layer):
    ...
    def call(self, inputs, verbose=False):
        ...
        def get_next_input(true, estimate):
            ...
            result1 = tf.scatter_nd(indices=where_sampling, updates=_estimate, shape=base_shape)
            result2 = tf.scatter_nd(indices=where_not_sampling, updates=_true, shape=base_shape)
        ...
        def gru_step(inputs, states):
            ...
            if self.sampling: 
                ...
                gru_outs, state = self.gru(next_input,initial_state=states) #=> (batch_size, hidden)
            else:
                estimate = self.output_dense(raw_input)
