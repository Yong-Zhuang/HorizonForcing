...
from tensorflow.keras.layers import RNN, GRUCell, GRU, Lambda, Multiply, Add, Concatenate, Dropout
...
class BaseModel:
    def __init__(self, dim=1, saved_folder="", model_name = "base", latent_dim=256, verbose=False,x_y_lag=1):  
        """        
        Args:
    ...
    def get_callbacks(self):        
        ...
        reduce_lr = ReduceLROnPlateau(monitor="val_loss", factor=0.1, patience=5, verbose=1,min_delta=1e-9)     
    ...
    def fit_generator(self, batch_size, epochs, callbacks, datagenerator_train, datagenerator_val, num_train_samples, num_val_samples):
    ...
    def training_generator(self, batch_size, epochs, datagenerator_train=None, datagenerator_val=None, num_train_samples= 0, num_val_samples=0):
        callbacks = self.get_callbacks()       
        return self.fit_generator(batch_size, epochs, callbacks, datagenerator_train, datagenerator_val, num_train_samples, num_val_samples)
    ...
    def inference(self, input_seq,pred_steps, model_path = None):
        ...
        Args:
            ...
            x_y_lag: time step gap between input x and output y
        """    
        assert len(input_seq.shape) >= 3, "input sequence should be greater 3 dimensions: (samples, time steps, other dimensions)"
        input_steps = input_seq.shape[1]
        assert self.x_y_lag <= input_steps, "time step gap between input x and output y should be smaller than the number of steps of input sequence, otherwise, inference process cannot be continue."
...
class TeacherForcing(BaseModel):
    def __init__(self, dim=1, saved_folder="", model_name = "teacher_forcing", latent_dim=256, verbose=False,x_y_lag=1):    
        super(TeacherForcing, self).__init__(dim, saved_folder, model_name, latent_dim, verbose,x_y_lag)  
        inputs = Input(shape=(None, self.dim), name="input") 
        self.gru = RNN(GRUCell(self.latent_dim),return_state=True, return_sequences=True, name="gru")     
...
class DiscontinuousScheduledSampling(BaseModel):
    def __init__(self, dim=1, saved_folder="", model_name = "discontinuous_scheduled_sampling", latent_dim=256, verbose=False,x_y_lag=1, ts= 0, batch_size = 0, decay="is"): 
        """        
        Args:
            ts: time steps of the input. 
        """        
        super(DiscontinuousScheduledSampling, self).__init__(dim, saved_folder, f"{model_name}_{decay}", latent_dim, verbose,x_y_lag)
        ...
        self.ss_gru = ScheduledSamplingLayer("ss_gru", self.gru, self.output_dense,self.dim,self.latent_dim,self.beta,self.batch_size )
        ...
        decoder_state_input = Input(shape=(self.latent_dim,))
        decoder_inf_out, decoder_inf_state = self.gru(inputs, initial_state=decoder_state_input)
        decoder_pred = self.output_dense(decoder_inf_out)
        self.decoder_model = Model([inputs, decoder_state_input], [decoder_pred, decoder_inf_out, decoder_inf_state])
    ...
    def training(self, X_train, y_train, X_val, y_val, batch_size, epochs):
        ...
        callbacks, history = self.fit(X_train, y_train, X_val, y_val, batch_size, epochs, callbacks)  
        self.model.save_weights(self.MODEL_PATH)  
        return callbacks, history
        

class SuperHorizonForcing(BaseModel):
    def __init__(self, dim=1, saved_folder="", model_name = "super_horizon_forcing", latent_dim=256, verbose=False,x_y_lag=1, k=10,lr=0.00000000001,suffix = ""): 
        """        
        Args:
            k: number of horizon steps for training. 
            lr: init learning rate for the model training
        """           
        super(SuperHorizonForcing, self).__init__(dim, saved_folder, f"{model_name}_{k}{suffix}", latent_dim, verbose,x_y_lag)
        self.k = k
        self.lr = lr
        inputs = Input(shape=(None, self.dim), name="input")
        ...
        outputs = []       
        self.time_gru = HorizonForcingGRULayer(f"time_gru", self.gru, self.output_dense,self.dim)
        ...
        adam = optimizers.Adam(learning_rate=self.lr,clipnorm = 1.)
        self.model.compile(loss=["mse"], optimizer=adam, metrics=["mae"])
        # if self.verbose:
        # self.model.summary()
        ...
        decoder_state_input = Input(shape=(self.latent_dim,))
        decoder_outputs, decoder_states = self.gru(inputs, initial_state=decoder_state_input)
        decoder_y_pred = self.output_dense(decoder_outputs)
        self.decoder_model = Model(
            [inputs, decoder_state_input],
...
class HorizonForcing(BaseModel):
    def __init__(self, dim=1, saved_folder="", model_name = "horizon_forcing", latent_dim=256, verbose=False,x_y_lag=1, k=10,lr=0.00000000001,suffix = ""): 
        """        
        Args:
            k: number of horizon steps for training. 
            lr: init learning rate for the model training
        """           
        super(HorizonForcing, self).__init__(dim, saved_folder, f"{model_name}_{k}{suffix}", latent_dim, verbose,x_y_lag)
        self.k = k
        self.lr = lr
        inputs = Input(shape=(None, self.dim), name="input")
        self.gru = RNN(GRUCell(self.latent_dim),return_state=True, return_sequences=True, name="gru")
...
class InverseSigmoidCallback(Callback):
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        #     self.best = current
        update = max(self.min_rate,self.k/(self.k+math.exp(self.decay_step/self.k)))
        K.set_value(self.beta, update) 
        self.decay_step += 1
    def on_epoch_end(self, epoch, logs={}):
        print(f"decay_step {self.decay_step}: {K.get_value(self.beta)*100}% usage of ground truth input. ")
...
class MusicTransformer(BaseModel):   
    def __init__(self,dim=1, saved_folder="", model_name = "music_transformer", latent_dim=256, verbose=False,x_y_lag=1, num_layer=6, max_seq=100, dropout=0.0, save_config=True):
    # (self,embedding_dim=256, num_layer=6,x_dim=3,
    #              max_seq=100, dropout=0.0, save_config=True, config_path=None, model_name = "MT"):
        super(MusicTransformer, self).__init__(dim, saved_folder, model_name, latent_dim, verbose,x_y_lag)
        ...
        self.Decoder = Encoder(
            num_layers=self.num_layer, d_model=self.latent_dim, rate=dropout, max_len=self.max_seq)
    ...
    def training(self, X_train, y_train, X_val, y_val, batch_size, epochs, fold = 0):
        ...
        callbacks, history = self.fit(X_train+ [train_mask], y_train, X_val+[val_mask], y_val, batch_size, epochs, callbacks)       
        if self.save_config:
    ...
    def inference(self, input_seq, pred_steps=2048):
        ...
        for i in Bar('inferring').iter(range(int(pred_steps/self.x_y_lag)+1)):
            if i % 100 == 0:
                print(f'{self.model_name} inferring... {(i/pred_steps)*100}% completed')
            look_ahead_mask = \
                self.get_masked_with_pad_tensor(sequence[:,-input_steps:,:])
    ...
    def get_masked_with_pad_tensor(self, inputs):    
        ...
        m = ~(np.ones((inputs.shape[1],inputs.shape[1])).cumsum(axis=1).T <= array_len).T
        m = m.astype(int)
